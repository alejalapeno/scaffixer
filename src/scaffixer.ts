...
async function* walkDirectory(
  ...
  for await (const file of await fs.promises.readdir(directory, {
    withFileTypes: true,
  })) {
    const { name } = file;
...
const findClosestDirectory = async (filepath: string): Promise<string> => {
  return await fs.promises.stat(filepath).then((file) => {
...
const getContents = async (
  filepath: string
): Promise<AsyncGenerator<FileInfo, void, undefined>> => {
  const stats = await fs.promises.stat(filepath).catch((err) => {
    window.showErrorMessage(
      `Cannot read or find file/directory at 'location'${filepath}`
    );
    throw new Error(
      `Cannot read or find file/directory at 'location'${filepath}`
    );
...
const engineEJS = (string: string, inputs: object): Promise<string> => {
  return renderPromise(string, inputs).catch((err) => {
    window.showErrorMessage(
      `Template contains a variable that is missing from the prompts: ${err}`
    );
...
const writeOutput = (
  { name, contents, isDirectory }: FileInfo,
  ...
  fse.outputFile(outputName, contents).catch((err: Error) => {
    console.error("Scaffixer Error:", err);
  });
};

export const scaffix = async (
  filepath: string,
  inputData: object,
  outputPath: string
) => {
  ...
  if (!inputData) {
    fse.copy(filepath, outputPath);
  } else {
    const ejsHelperPath = path.join(filepath, "./ejs_helpers.js");
    try {
      const ejs_helpers = require(ejsHelperPath);
      inputData = { ...inputData, _: ejs_helpers };
    } catch (error) {}
    try {
      for await (const file of await getContents(filepath)) {
        const { name, contents, isDirectory } = file as FileInfo;
        if (name === "ejs_helpers.js") {
          continue;
        }
      ...
      delete require.cache[ejsHelperPath];
    }
  }
};
